...
cleanup() {
    ...
    [[ -n "$BUILD_DIR" && -d "$BUILD_DIR" ]] && remove_dir "$BUILD_DIR" || true
}
...
# Scratch build directories can end up holding a few root-owned files from
# `sudo ... install`.
remove_dir() { rm -rf "$1" 2>/dev/null || sudo rm -rf "$1"; }
...
build_gpu_screen_recorder() {
    ...
    # The same options as its install.sh (now deprecated), but built as you
    # and only installed as root: install.sh ran everything as root, leaving
    # a build directory you couldn't delete.
    meson setup "$dir/gsr/build" "$dir/gsr" \
        --prefix=/usr --buildtype=release -Dstrip=true -Dsystemd=true
    meson compile -C "$dir/gsr/build"
    sudo meson install -C "$dir/gsr/build"
...
check_nvidia_modules() {
    ...
    # Only kernel version directories: /lib/modules can hold others (one
    # called "kernel" made this check warn about a kernel that doesn't exist).
    local newest
    newest=$(find /lib/modules -mindepth 1 -maxdepth 1 -type d -name '[0-9]*' -printf '%f\n' \
        | sort -V | tail -n 1)
    [[ -n "$newest" ]] || { warn "no kernels found in /lib/modules"; return 0; }
...
update() {
    ...
    if [[ -f /var/run/reboot-required ]]; then
        info "Ubuntu says a reboot is needed (new kernel, drivers or core libraries)."
    elif _in hyprland "${ok[@]}"; then
        info "Log out and back in to start the new Hyprland."
    fi
